//! Provides tools for resolving the path of modules.
...
pub fn resolve_path(name: String, src: String) -> Option<PathBuf> {
    ...
    if !tmp_name.ends_with(".fly") {
        tmp_name.push_str(".fly");
    }
    ...
    if p.exists() {
        ...
        let mod_path = exe.join("modules");
        let folder = mod_path.join(Path::new(&src));
        
        if folder.exists() {
            let module_file = folder.join(Path::new("mod.fly"));
